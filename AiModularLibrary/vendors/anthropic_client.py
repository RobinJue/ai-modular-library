...
class AnthropicClient:
    def __init__(self, logger=None):
        ...
        if not api_key:
            error_msg = "ANTHROPIC_KEY not found in secrets.env"
            if self.logger:
                self.logger.error(error_msg)
            raise ValueError(error_msg)
        
        if self.logger and not api_key.strip():
            self.logger.warning("ANTHROPIC_KEY appears to be empty or whitespace only")
    ...
    def _get_model_info(self, model_name: str) -> Dict[str, Any]:
        ...
        if model_info['vendor'] != 'Anthropic':
            error_msg = f"Model '{model_name}' is not an Anthropic model"
            if self.logger:
                self.logger.error(error_msg)
            raise ValueError(error_msg)
    ...
    def call_api(self, model_name: str, prompt: str, 
        ...
        """
        Call Anthropic API with the specified parameters.
        
        Args:
            model_name: Name of the model (e.g., 'claude3haiku', 'claude3opus')
            prompt: User prompt
        ...
        if max_tokens is None:
            max_tokens = 1000  # Default to 1000 tokens
